...
// unsigned int optimize_intervals_and_compute_mean_intervals_float_1D(float *oriData, size_t dataLength, double realPrecision, float * dense_pos){
...
// 	size_t * freq_intervals = (size_t *) malloc(range*sizeof(size_t));
// 	memset(freq_intervals, 0, range*sizeof(size_t));
// 	float * freq_sum = (size_t *) malloc(range*sizeof(size_t));
// 	memset(freq_sum, 0, range*sizeof(size_t));
// 	// compute optimized intervals
// 	size_t i = 0, radiusIndex;
// 	float pred_value = 0, pred_err;
// 	size_t *intervals = (size_t*)malloc(maxRangeRadius*sizeof(size_t));
...
unsigned int optimize_intervals_and_compute_dense_position_float_3D(float *oriData, size_t r1, size_t r2, size_t r3, double realPrecision, float * dense_pos){	
	...
	*dense_pos = mean + realPrecision * (ptrdiff_t)(max_index - 1 - radius);
	printf("real precision: %.4f dense_pos: %.4f max_index: %ld\n", realPrecision, dense_pos[0], max_index);
...
// NOTE: BLOCK_DIM is assumed to be less than int range
unsigned char * SZ_compress_float_1D_MDQ_RA(float *oriData, size_t r1, double realPrecision, size_t * comp_size){
	...
	// NOTE: BLOCKSIZE is unsigned short now, it may not be suitable for blocks with over 8192 elements
...
unsigned int SZ_compress_float_2D_MDQ_RA_block_2D_pred(float * block_ori_data, float * mean, float dense_pos, size_t dim_0, size_t dim_1, int block_dim_0, int block_dim_1, double realPrecision, float * P1, int * type, float * unpredictable_data){
	...
	int num_elements = block_dim_0 * block_dim_1;
	mean[0] = sum / mean_count;
...
#define COMPUTE_2D_NUMBER_OF_BLOCKS( COUNT, NUM_BLOCKS ) \
    if (COUNT <= 32){					\
    	NUM_BLOCKS = 1;				\
    }									\
    else{								\
...
unsigned char * SZ_compress_float_2D_MDQ_RA(float *oriData, size_t r1, size_t r2, float realPrecision, size_t * comp_size){
	...
	size_t num_x, num_y;
	COMPUTE_2D_NUMBER_OF_BLOCKS(r1, num_x);
	COMPUTE_2D_NUMBER_OF_BLOCKS(r2, num_y);

	size_t split_index_x, split_index_y;
	int early_blockcount_x, early_blockcount_y;
	int late_blockcount_x, late_blockcount_y;
	COLL_BASE_COMPUTE_BLOCKCOUNT(r1, num_x, split_index_x, early_blockcount_x, late_blockcount_x);
	...
	// exit(0);
	int max_num_block_elements = early_blockcount_x * early_blockcount_y;
	...
	float *P1; // buffer
	// NOTE: Allocat b1 * b2 buffer to store prediction, should change to 2 * b2 buffer later
	size_t buffer_size = early_blockcount_x * early_blockcount_y * sizeof(float);
	...
	int current_blockcount_x, current_blockcount_y;
	size_t type_offset = 0;
	...
	for(size_t i=0; i<num_x; i++){
		for(size_t j=0; j<num_y; j++){
			...
			unpredictable_count[index] = SZ_compress_float_2D_MDQ_RA_block_2D_pred(data_pos, mean + index, dense_pos, r1, r2, current_blockcount_x, current_blockcount_y, realPrecision, P1, type, unpredictable_data);
	...
	for(size_t i=0; i<num_x; i++){
		for(size_t j=0; j<num_y; j++){
			...
			type = result_type + type_offset;
			int current_block_elements = current_blockcount_x * current_blockcount_y;
			enCodeSize = 0;
...
// unsigned short SZ_compress_float_3D_MDQ_RA_block_3D_pred_with_mean_intervals(float * block_ori_data, float * means, size_t dim_0, size_t dim_1, size_t dim_2, int block_dim_0, int block_dim_1, int block_dim_2, double realPrecision, float * P0, float * P1, int * type, float * unpredictable_data){
...
// unsigned char * SZ_compress_float_3D_MDQ_RA_with_mean_intervals(float *oriData, size_t r1, size_t r2, size_t r3, float realPrecision, size_t * comp_size){

// 	unsigned int quantization_intervals;
// 	unsigned int mean_intervals;
// 	float dense_pos = realPrecision;
// 	float * seg_means;
// 	if(optQuantMode==1)
// 	{
// 		// quantization_intervals = optimize_intervals_float_1D(oriData, r1, realPrecision);
// 		quantization_intervals = optimize_intervals_and_compute_mean_intervals_float_3D(oriData, r1, r2, r3, realPrecision, &dense_pos, &mean_intervals, &seg_means);
// 		// quantization_intervals = 65536;
// 		printf("number of bins: quantization %d mean %d\nerror bound %.12f dense position %.12f\n", quantization_intervals, realPrecision, dense_pos);
// 		// dense_pos = realPrecision;
// 		updateQuantizationInfo(quantization_intervals);
// 		intvCapacity = quantization_intervals - 2 * mean_intervals;
// 	}	
// 	else{
// 		quantization_intervals = intvCapacity;
// 		intvCapacity = quantization_intervals - 2 * mean_intervals;
...
// 				unpredictable_count[index] = SZ_compress_float_3D_MDQ_RA_block_3D_pred(data_pos, means, dense_pos, r1, r2, r3, current_blockcount_x, current_blockcount_y, current_blockcount_z, realPrecision, P0, P1, type, unpredictable_data);
...
// 	intToBytes_bigEndian(result_pos, quantization_intervals);
// 	result_pos += 4;
// 	intToBytes_bigEndian(result_pos, mean_intervals);
// 	result_pos += 4;
// 	memcpy(result_pos, means, mean_intervals * sizeof(float));
// 	result_pos += mean_intervals * sizeof(float);
// 	intToBytes_bigEndian(result_pos, treeByteSize);
// 	result_pos += 4;
// 	intToBytes_bigEndian(result_pos, nodeCount);
// 	result_pos += 4;
...
unsigned char * SZ_compress_float_3D_MDQ_RA(float *oriData, size_t r1, size_t r2, size_t r3, float realPrecision, size_t * comp_size){
	...
	// int unpredictable_count = 0;
	unsigned short * unpredictable_count = (unsigned short *) malloc(num_blocks * sizeof(unsigned short));
	float * mean = malloc(num_blocks * sizeof(float));
	size_t total_unpred = 0;
	size_t index = 0;
	int max_unpred_count = 0;
